...
import { GraphQLID, GraphQLInputObjectType, GraphQLInterfaceType, GraphQLList, GraphQLNonNull, GraphQLObjectType, GraphQLSchema, GraphQLString, buildSchema } from "graphql"
...
const registerEndUserResolver: GraphQLFieldResolver<GraphQLContext, {}> = async (ctx, { input }) => {
  console.log(input)
  ...
  try {
    const result = await db.insert(users).values({ id: input.id, name: input.name }).returning({ id: users.id, name: users.name, profileImageUrl: users.profileImageUrl})
